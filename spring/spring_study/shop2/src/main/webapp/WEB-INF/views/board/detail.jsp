...
<%-- /WEB-INF/views/board/detail.jsp 
	http://localhost:8080/board/detail?num=1 요청
	  - service.getBoard(num)
	    ...
	    num 파라미터에 해당 게시물의 정보를 db에서 읽어 Board 객체에 저장
	  - 조회 수 증가
...
<body>
 ...
 <%-- 댓글 등록, 조회, 삭제 --%>
 <%-- http://localhost:8080/detail?num=#comment 요청
 ...
 <div class="w3-row">
   ...
   <div class="w3-col s2 w3-center">
    <p><button type="submit" class="w3-btn w3-boarder">댓글등록</button>
       </p>
   </div></div>
   </form:form>
    ...
    <table class="w3-table-all">
     <c:forEach var="c" items="${commlist}" varStatus="stat">
      ...
      <form action="commdel" method="post" name="commdel${stat.index}">
       ...
       <a class="w3-bten w3-border w3-blue"
