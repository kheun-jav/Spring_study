...
/*
 	...
 	2. 클래스에서 사용되는 어노테이션
 	  ...
 	  @Autowired : 자료형 기준으로 객체 주입. 주입대상의 객체가 없으면 오류 발생
 	  			   객체 주입 필수
 	  @Autowired(required=false) : 주입대상의 객체가 없으면 null로 주입함.
 	  			   객체 주입 선택
 	  @Scope : 일회용 객체로 생성. 사용될때마다 새로운 객체로 생성함
 	3. AOP 관련 어노테이션
 	  @Aspect : AOP로 사용될 클래스로 지정(Component 지정 필수)
 	  @Order(순서) : 순서는 before 기준. after의 경우는 order의 역순으로 실행됨. 
 	  
 	  @Before : 핵심 기능 실행 이전 
 	  @AfterReturning : 핵심 기능 정상 실행 후. 리턴값 조회 가능
 	  @AfterThrowing  : 핵심 기능 오류 실행 후. 예외값 조회 가능
 	  @After : 핵심 기능 실행 이후
 	  @Around : 핵심 기능 실행 이전/이후 모두 처리
 */
public class Main1 {
