...
public class ShopService {
	...
	public void itemCreate(Item item, HttpServletRequest request) {
		...
		item.setId(maxid + 1); //새로 등록할 상품의 id
		itemDao.insert(item);
	}
	//파일 업로드하기
	private void uploadFileCreate(MultipartFile picture, String path) {
		...
		try {
			//picture : 파일의 내용
			//trandferTo : picture의 내용을 new File의 위치로 저장
	...
	public List<Sale> saleList(String userid) {
		...
		List<Sale> list = saleDao.list(userid); //userid 사용자 주문 정보 목록
		for(Sale sa : list) { 
		...
		return list; //db정보, SaleItem(주문상품)정보 저장
	}
}
