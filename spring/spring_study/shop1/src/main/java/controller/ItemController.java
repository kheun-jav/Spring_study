...
public class ItemController {
	...
	@RequestMapping("list") //Get, Post 방식 요청시 호출(상관없음)
	public ModelAndView list() {
		//ModelAndView : View에 이름과 전달 데이터를 저장
		ModelAndView mav = new ModelAndView();
		//itemList : item 테이블의 모든 정보를 저장하고 있는 객체
		List<Item> itemList = service.itemList();
		mav.addObject("itemList",itemList); //veiw에 전달할 객체 저장
//		mav.setViewName("item/list"); view이름이 null인 경우 요청 URL에서 가져옴
		return mav;
	}
	...
	//Integer id : id 라는 이름을 가진 파라미터값을 저장. 매개변수명 = 파라미터명
	...
	/*
	 * Item item : 파라미터의 이름과 item 클래스의 setProperty를 이용하여
	 * 			   파라미터 값 저장
	 * 	   Item item = new Item();
	...
	public ModelAndView register(@Valid Item item, BindingResult bresult,
		...
		if(bresult.hasErrors()) { //입력값 검증시 오류 발생
			return mav; //url이 같은 view로 이동(create.jsp)
		}
		//입력값이 정상인 경우
	...
	public ModelAndView update(@Valid Item item, BindingResult bresult,
		...
		/*
		 * item : id, name, price, description, pictureUrl
		 */
